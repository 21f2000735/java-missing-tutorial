...
const sharedStaticStyle = `
  ...
  *{box-sizing:border-box}
  ...
  html[data-theme="dark"] .theme-toggle{background:rgba(40,34,28,.86);box-shadow:inset 0 0 0 1px var(--line)}
  .nav a:hover,.theme-toggle:hover{transform:translateY(-1px)}
  .nav a:focus-visible,.theme-toggle:focus-visible,.back-link:focus-visible{outline:2px solid var(--accent-strong);outline-offset:3px}
  .layout{display:grid;grid-template-columns:minmax(0,1fr) minmax(18rem,24rem);gap:1rem}
  .code-title,.lead,.muted{color:var(--muted)}
  img{display:block;width:min(100%,56rem);margin:1.2rem auto;border-radius:1.15rem;border:1px solid var(--line);background:var(--panel-strong);box-shadow:var(--shadow)}
  pre{overflow:auto;background:#241d15;color:#fdf8f1;padding:1rem;border-radius:1rem;font-size:.92rem;line-height:1.55}
  ...
  table{width:100%;border-collapse:collapse;margin:1rem 0;border-radius:1rem;overflow:hidden}
